...
foreach(this.attribs??[] as $attrib) { 
   ...
   `; foreach($attrib["attrib_value"]??[] as $av) { `
                              ...
                              <div class="form-check mb-2 mt-1 ms-1">
                                 <!-- input -->
                                 <input class="form-check-input form-check-input-filter-av" type="checkbox" value="`$av["content"] `" id="apav_`$attrib["attrib_id"] `" `(array_key_exists($attrib["attrib_id"], this.pvav) && in_array($av["content"], this.pvav[$attrib["attrib_id"]])) ? "checked" : "" ` >
...
`;}                                 <!-- <a id="productListViewAnchor" href="" class="text-muted me-3 productViewSwitch"><i id="productListView" class="bi bi-list-ul"></i></a>
                     ...
                     <!-- row -->
                     <div id="productViewContainer" class="`(this.options['productViewType'] == 'grid') ? "row g-4 row-cols-xxl-6 row-cols-xl-6 row-cols-lg-4 row-cols-2 row-cols-md-2 mt-2 product-view-container": "row g-4 row-cols-1 mt-2 product-view-container"; `">
...
yield html`
                     ...
                     <div id="paginationContainer">
                        ${this.paginationHtml}`; //this.generateSmartPagination(this.options["currentPage"], this.options["totalPages"], 5); `
