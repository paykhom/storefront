            ...
            class Page extends UserLayout {
                ...
                async uponReady() {
                    ...
                    let table = new DataTable(
                        '#myDataTable',
                        {
                            ...
                            ajax: '/api/dtx/ecom/buy_payment/search', // Replace with your actual API endpoint
                            processing: false,
                            serverSide: true,
                            pageLength: 25,
                            columns: [
					...
					$('#add-buy-payment-form').on('submit', async function (e) {
						...
						const apiURL = '/api/dtx/ecom/buy_payment/create';
						const method = 'POST';
                        try {
                ...
                async onClickDeleteRow(id) {
                    if (confirm("Are you sure you want to delete this payment?")) {
                        try {
                             const apiURL = ~/api/dtx/ecom/buy_payment/~{id}~;
                             const method = 'DELETE';
