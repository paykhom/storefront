...
export default class UserPaginator extends SaasLayout {
		...
		class Page extends SaasLayout {
								...
								render: function (data, type, row) {
									return ~
										<div class="edit-delete-action">
											<a class="me-2 edit-icon  p-2" href="/saas/catalog/user/editor/~{row.user_id}">
												<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="feather feather-edit">
													<path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"></path>
													<path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"></path>
												</svg>
											</a>
											<a class="confirm-text p-2" href="#" onclick="page.onClickDeleteRow(this, ~{row.user_role_id})">
			...
			async onClickDeleteRow(e, id) {
                if(confirm("Are you sure you want to delete this user role?")) {
                   let r = await (await fetch(~/saas/user-role/delete/~{id}~, {method: 'DELETE'})).json();
                    if(r.success) {
                       alert("User role deleted.");
                       window.location.reload()
                   } else {
                       alert("User role could not be deleted.");
                   }
                }
			}			
		}
