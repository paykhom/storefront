...
export default class AttribRequestPaginator extends AdminLayout {
        ...
        <div class="page-header">
            <div class="add-item d-flex">
                <div class="page-title">
                    <h4>Attribute Requests</h4>
                    <h6>Manage your attributes</h6>
                </div>
            </div>
            ...
            async uponReady() {
                ...
                let table = new DataTable(
                    '#myDataTable',
                    {
                        ...
                        ajax: '/api/dtx/ecom/attrib/search',
                        processing: false,
                        serverSide: true,
                        pageLength: 25,
                        columns: [
								...
								render: function (data, type, row) {
									return ~
										<div class="edit-delete-action">
											...
											<a class="me-2 p-2" href="/admin/catalog/attrib-request/editor/~{row.attrib_id}">
                       ...
                       initComplete: function() {
                            // Append a combobox (select element) to the header of the 'Publication' column
                            this.api().columns(8).every(function() {
                                var column = this;
                                var select = $('<select class="form-select"><option value="">Publication Status</option></select>')
                                ...
                                let pubs = [
                                    {"publication_status_id": 1, "title": "Published"},
                                    {"publication_status_id": 2, "title": "Draft"},
                                ];
                                pubs.forEach(c => {
                                    select.append('<option value="' + c.publication_status_id + '">' + c.title + '</option>');
                                });
                            });
                        }
                    }
