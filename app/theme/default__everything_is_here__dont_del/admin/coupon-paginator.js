...
export default class CouponPaginator extends AdminLayout {
            ...
            async uponReady() {
                ...
                let table = new DataTable(
                    '#myDataTable',
                    {
                        ...
                        ajax: '/api/dtx/ecom/coupon/search',
                        processing: false,
                        serverSide: true,
                        pageLength: 25,
                        columns: [
                            ...
                            { data: 'coupon_id', title: "ID" },
                            { data: 'code', title: "Code" },
                             { data: 'discount_type', title: "Discount Type" },
                            { data: 'discount_value', title: "Discount Value" , render: $.fn.dataTable.render.number(',', '.', 2) },
                            { data: 'min_purchase_amount', title: "Min Purchase", render: $.fn.dataTable.render.number(',', '.', 2) },
                            { data: 'max_discount_amount', title: "Max Discount", render: $.fn.dataTable.render.number(',', '.', 2) },
                           { data: 'start_date', title: "Start Date" },
                            { data: 'end_date', title: "End Date" },
                            { data: 'usage_limit', title: "Usage Limit" },
                            { data: 'usage_count', title: "Usage Count"},
                            {
                                 data: 'is_active',
                                 title: 'Active',
                                ...
                                render: function (data, type, row) {
                                    return ~
                                        <div class="edit-delete-action">
                                            ...
                                            <a class="me-2 p-2" href="/admin/marketing/coupon/editor/~{row.coupon_id}">
                        ...
                        initComplete: function() {
                            // Append a combobox (select element) to the header of the 'is_active' column
                            this.api().columns(11).every(function() {
                                var column = this;
                                var select = $('<select class="form-select"><option value="">Active Status</option></select>')
                                ...
                                let status = [
                                    {"is_active": true, "title": "Yes"},
                                    {"is_active": false, "title": "No"},
                                ];
                                status.forEach(s => {
                                    select.append('<option value="' + s.is_active + '">' + s.title + '</option>');
                                });
                            });
                        }
                    }
