...
export default class StockLedgerPaginator extends AdminLayout {
					...
					<div class="page-header">
						<div class="add-item d-flex">
							<div class="page-title">
								<h4>Stock Ledgers</h4>
								<h6>Manage your stock ledgers</h6>
							</div>
						</div>
	...
	<script>
		class Page extends AdminLayout {
			...
			async uponReady() {
				...
				let table = new DataTable(
					'#myDataTable', 
					{
						...
						ajax: '/api/dtx/ecom/stock-ledger/search',
						processing: false,
						serverSide: true,
						pageLength: 25,
						columns: [
							{
                             ...
                             {
                                title: "Stock Ledger ID",
                                data: 'stock_ledger_id',
                            },
                            {
                                title: "Shop ID",
                                data: 'shop_id',
                            ...
                            {
                                title: "Stock Location ID",
                                data: 'stock_location_id',
                            },
                            {
                                title: "Transaction Type",
                                data: 'stock_transaction_type_id',
                            },
                            {
                                title: "Date",
                                data: 'date',
								...
								render: function (data, type, row) {
									return ~
										<div class="edit-delete-action">
											<a class="me-2 edit-icon  p-2" href="/admin/stock/ledger/editor/~{row.stock_ledger_id}">
												<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="feather feather-edit">
													<path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"></path>
													<path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"></path>
												</svg>
											</a>
											<a class="confirm-text p-2" href="#" onclick="page.onClickDeleteRow(this, ~{row.stock_ledger_id})">
			...
			async onClickDeleteRow(e, id) {
                if(confirm("Are you sure you want to delete this stock ledger?")) {
                    let r = await (await fetch(~/ecom/stock-ledger/delete/~{id}~, {method: 'DELETE'})).json();
                    if(r.success) {
                        alert("Stock ledger deleted.");
                        window.location.reload()
                    } else {
                       alert("Stock ledger could not be deleted.");
                    }
                }
			}			
		}
