...
`;  
  ...
  for (const item of product['inventory_detail'] ?? []) {
                                       ...
                                       <div class="d-flex justify-content-between align-items-center mt-3">
                                          ...
                                          <div>
                                             <a 
                                                ...
                                                data-product-variant-id="${product['inventory_id']}" 
                                 ...
                                 <div class="row align-items-center">
                                    ...
                                    <div class="col-md-8 col-12 flex-grow-1">
                                       ...
                                       <div class="mt-6">
                                          ...
                                          <div class="mt-2">
                                             <a href="#!" class="btn btn-primary pvs-btn-add-to-cart"  data-product-variant-id="${product['inventory_id']}" data-product-price="${product['price']}" data-product-title="${product['title']}" data-product-media-path="/theme/freshcart/assets/images/product-single-img-1.jpg" data-product-link-product-slug="${product['slug']}" data-product-list-price="${product['list_price']}">
   ...
   <script>
            ...
            if (this.args.type === PRODUCT_GRID_VIEW) {
               ...
               <div class="card card-product">
                  <div class="card-body">
                     ...
                     <div>
                        <a href="#!" class="btn btn-primary btn-sm pvs-btn-add-to-cart" data-product-variant-id="~{product.inventory_id}"  data-product-variant-id="~{product.inventory_id}" data-product-price="~{product.price}" data-product-title="~{product.title}" data-product-media-path="/theme/freshcart/assets/images/product-single-img-1.jpg" data-product-link-product-slug="~{product.slug}" data-product-list-price="~{product.list_price}">
                           <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="feather feather-plus">
                           <line x1="12" y1="5" x2="12" y2="19"></line>
                           <line x1="5" y1="12" x2="19" y2="12"></line>
                           </svg>
                                    ...
                                    <div class="col-md-8 col-12 flex-grow-1">
                                     ...
                                     <!--  <div>
                                       ...
                                       <div class="mt-6">
                                          ...
                                          <div class="mt-2">
                                             <a href="#!" class="btn btn-primary pvs-btn-add-to-cart"  data-product-variant-id="~{product.inventory_id}"  data-product-variant-id="~{product.inventory_id}" data-product-price="~{product.price}" data-product-title="~{product.title}" data-product-media-path="/theme/freshcart/assets/images/product-single-img-1.jpg" data-product-link-product-slug="~{product.slug}" data-product-list-price="~{product.list_price}">
                                                <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="feather feather-shopping-bag me-2">
                                                   <path d="M6 2L3 6v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2V6l-3-4z"></path>
                                                   <line x1="3" y1="6" x2="21" y2="6"></line>
                                                   <path d="M16 10a4 4 0 0 1-8 0"></path>
      ...
      class Page extends MarketplaceLayout {
         constructor(args) {
            ...
            this.inventory_detail = ${JSON.stringify(this.pvav)};
            if (Array.isArray(this.inventory_detail) && this.inventory_detail.length == 0) {
               this.inventory_detail = {};
            }
         }
         ...
         async uponReady() {
            ...
            slider.noUiSlider.on('update', async function (values, handle) {
               //alert("Yes");
               //page.productSearchParams.product.inventory.price_low =  parseFloat((values[0] || 1).replace(/[^0-9\.]/g, '')); ;
               //page.productSearchParams.product.inventory.price_high = parseFloat((values[1] || 999999).replace(/[^0-9\.]/g, '')); ;
               

               if (!page.productSearchParams.product) {
                  page.productSearchParams.product = {};
               } 
               if (!page.productSearchParams.product.inventory) {
                  page.productSearchParams.product.inventory = {};
               } 
         ...
         async onClickGlobalSearch(e) {
			...
			async onClickAddToCart(e) {
						...
						const inventory_id = Number(e.target.getAttribute('data-product-variant-id'));
						const quantity = 1;
         ...
         async attribFilterOnClick(ev) {
            ...
            // Ensure this.inventory_detail is defined
            if (!this.inventory_detail) {
               this.inventory_detail = {};
            }

            // Ensure attrib_id exists in the object

            if (!this.inventory_detail[attrib_id]) {
               this.inventory_detail[attrib_id] = [];
            }

            if (isChecked) {
               // Add value if not already in the array
               if (!this.inventory_detail[attrib_id].includes(value)) {
                     this.inventory_detail[attrib_id].push(value);
               }
            } 
            else {
               // Remove value if it exists in the array
               this.inventory_detail[attrib_id] = this.inventory_detail[attrib_id].filter(
                     (item) => item !== value
               );

               // Remove attrib_id key if the array is empty
               if (this.inventory_detail[attrib_id].length === 0) {
                     delete this.inventory_detail[attrib_id];
               }
            }

            if (!this.productSearchParams) {
               this.productSearchParams = { product: { inventory: {} } };
            } 
            else if (!this.productSearchParams.product) {
               this.productSearchParams.product = { inventory: {} };
            } 
            else if (!this.productSearchParams.product.inventory) {
               this.productSearchParams.product.inventory = {};
            }


            this.productSearchParams.product.inventory.inventory_detail = this.inventory_detail;
            this.productSearchParams.product.inventory.price_low = this.price_low;
            this.productSearchParams.product.inventory.price_high = this.price_high;
