...
export default class AddressPaginator extends UserLayout {
			...
			async uponReady() {
				...
				let table = new DataTable(
					'#myDataTable',
					{
						...
						xsDom: 'fBtlpi',
						xordering: true,
                        order: [[1, 'asc']],  // Sort by the second column (address_id) ascending
						xinitComplete: (settings, json)=>{
							$('.dataTables_filter').appendTo('#tableSearch');
							$('.dataTables_filter').appendTo('.search-input');
						},
						ajax: '/api/dtx/saas/address/search',
						processing: true,
						serverSide: true,
						pageLength: 25,
						columns: [
							...
							{ data: 'address_id', title: "Address ID" },
							{ data: 'first_name', title: "First Name" },
                            { data: 'last_name', title: "Last Name"},
							{ data: 'address_type', title: "Address Type" },
							{ data: 'line1', title: "Address Line 1" },
                            { data: 'line2', title: "Address Line 2"},
							...
							{ data: 'mobile', title: "Mobile" },
                            {
                                ...
                                render: function (data, type, row) {
                                    // Assuming you have some way to fetch the username based on user_id
                                    // This part will need to be adjusted based on your setup to retrieve username
								...
								render: function (data, type, row) {
									return ~
										<div class="edit-delete-action">
											...
											<a class="me-2 p-2" href="/user/account/address/editor/~{row.address_id}">
                         ...
                         initComplete: function() {
							// Append a combobox (select element) to the header of the 'address_type' column
							this.api().columns(4).every(function() {
								var column = this;
								var select = $('<select class="form-select"><option value="">All Types</option></select>')
